...
Artifacts summary.
The repository includes (i) full training and inference for GPT-OSS-20B; (ii) the fine-tuned checkpoint; (iii) Appendix E synthetic simulation code;
...
\subsection{Stability Regularizer (\texorpdfstring{\(\mathcal{L}_{\text{Stab}}\)}{L\_Stab})}
...
\subsubsection{Empirical Estimation of \texorpdfstring{\(L_u\)}{L\_u}}
...
\subsubsection{Practical Bounds and Validation}

Theoretical estimates based on spectral norm analysis of the notes pathway yield the following order-of-magnitude ranges for \(L_u\) as a function of the notes embedding dimension: small (256): \(L_u\approx\mathcal{O}(10)\); medium (512): \(L_u\approx\mathcal{O}(20)\); large (1024): \(L_u\approx\mathcal{O}(50)\). These estimates assume standard weight initialization and LayerNorm on the notes input; empirical measurement via the layer-wise SDP estimator (Section~\ref{app:safeguards}) is recommended before deploying with large \(d_{\text{notes}}\).

Validation procedures include perturbation tests, gradient-norm lower bound \(L_u^{\text{grad}}\), and adversarial worst-case probing within an \(\epsilon\)-ball around representative notes vectors.
...
\subsection{Degradation Patterns and Complexity (from \S4.5)}
...
\subsection{Hierarchical Grouping Strategy (N=9 Example)}
...
\subsection{Practical Operating Points (Illustrative)}
For the fixed bus:
\begin{itemize}
...
\subsection{Degradation Mitigation Strategies}
\begin{itemize}
...
\subsection{Deployment Heuristics}
\begin{itemize}
...
\subsection{Paging Layout \texorpdfstring{(Paged-KV \cite{Kwon2023}; Rollback-aware)}{(Paged-KV; Rollback-aware)}}
...
See \texttt{scripts/logit\allowbreak\_replay.py} documentation for artifact generation and replay procedures.
