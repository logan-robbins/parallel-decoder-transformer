...
H100 optimization, and every empirical result remain pending.
...
13. [in progress] Build the missing planner-supervision corpus before claiming the planner gate. Ground every example in immutable long-form encyclopedic or report-like source documents, then use one joint Batch API teacher request to emit a shared expository prompt, exactly three unordered complementary stream plans, three several-paragraph stream targets, explicit cross-stream dependency links, a coverage map, and source-span provenance. Preserve paragraphs and discourse structure; do not convert the task into short QA or independent per-stream responses. Validate the schema and content locally, split by source/topic, randomly permute stream order, and train/evaluate decomposition with permutation-invariant matching so fixed labels cannot masquerade as learned planning.
