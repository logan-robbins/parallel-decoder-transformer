...
def test_codebook_reports_observable_ceiling_and_effective_entries() -> None:
    diagnostics = CodebookDiagnostics(vocab_size=8, num_slots=2)
    ...
    assert stats.effective_entries_per_slot == pytest.approx([2.0, 1.0])
    assert stats.exactly_collapsed is False


def test_codebook_marks_only_exact_single_entry_collapse() -> None:
    diagnostics = CodebookDiagnostics(vocab_size=8192, num_slots=3)
