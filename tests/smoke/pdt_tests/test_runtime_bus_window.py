...
def test_cached_scheduler_never_refeeds_prompt_and_publishes_tau_token_hidden() -> None:
    ...
    class FakeTrunk:
        def __init__(self, layer: FakeLayer) -> None:
            ...
            self.singleton_masks: list[tuple[int, list[list[bool]]]] = []
            self.cached_call_masks: list[tuple[list[int], list[list[bool]]]] = []
        ...
        def forward(self, *, input_ids, past_key_values=None, **_kwargs):
            ...
            if past_key_values is not None:
                ...
                self.cached_call_masks.append(
                    (input_ids[0].tolist(), self.layer.context.notes_mask.tolist())
                )
            ...
            prior = 0 if past_key_values is None else int(past_key_values)
            return SimpleNamespace(
                hidden_states=(hidden,),
                logits=logits,
                past_key_values=prior + input_ids.size(1),
            )
    ...
    # Three full-prefill calls (planner + K streams), then each generated
    # token is consumed exactly once. The final prompt token 6 is never fed as
    # a singleton.
    ...
    assert ([20, 21], [[True, True, True, True]]) in (model.trunk_adapter.fake.cached_call_masks)
    assert ([30, 31], [[True, True, True, True]]) in (model.trunk_adapter.fake.cached_call_masks)
...
def test_lww_read_is_order_independent_and_idempotent() -> None:
    ...
    old = bus.publish("stream_0", torch.full((3,), 3.0), published_block=0, stride=4)
    latest = bus.publish("stream_0", torch.full((3,), 4.0), published_block=1, stride=8)
...
def test_source_swap_changes_only_receiver_sibling_dynamic_payloads() -> None:
    ...
    with pytest.raises(ValueError, match="exact null"):
        apply_source_swap(
            notes,
            mask,
            anchor_mask=torch.tensor([True, True, True, False, False, False]),
...
def test_bus_mutation_is_targeted_deterministic_and_non_aliasing() -> None:
    ...
    mutated = apply_bus_mutation(original, magnitude=0.5)

    assert original.tolist() == [1.0, 2.0, 3.0]
    assert mutated.tolist() == [1.5, 2.0, 3.0]
    with pytest.raises(ValueError, match="finite and non-zero"):
        apply_bus_mutation(original, magnitude=0.0)


def test_unaddressed_snc_producer_permutation_is_an_exact_null() -> None:
    torch.manual_seed(7)
    snc = SharedNotesCrossAttention(SNCConfig(hidden_size=8, notes_dim=4, num_heads=2)).eval()
    ...
    original = snc(hidden, notes, notes_mask=mask, force_gate=True)
    permuted = snc(
        hidden,
        notes[:, [2, 0, 1]],
